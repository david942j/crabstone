...
module TestARM
  ...
  ARM_CODE     = "\xED\xFF\xFF\xEB\x04\xe0\x2d\xe5\x00\x00\x00\x00\xe0\x83\x22\xe5\xf1\x02\x03\x0e\x00\x00\xa0\xe3\x02\x30\xc1\xe7\x00\x00\x53\xe3\x00\x02\x01\xf1\x05\x40\xd0\xe8\xf4\x80\x00\x00".freeze
  ARM_CODE2    = "\xd1\xe8\x00\xf0\xf0\x24\x04\x07\x1f\x3c\xf2\xc0\x00\x00\x4f\xf0\x00\x01\x46\x6c".freeze
  THUMB_CODE   = "\x70\x47\xeb\x46\x83\xb0\xc9\x68\x1f\xb1\x30\xbf\xaf\xf3\x20\x84".freeze
  THUMB_CODE2  = "\x4f\xf0\x00\x01\xbd\xe8\x00\x88\xd1\xe8\x00\xf0\x18\xbf\xad\xbf\xf3\xff\x0b\x0c\x86\xf3\x00\x89\x80\xf3\x00\x8c\x4f\xfa\x99\xf6\xd0\xff\xa2\x01".freeze
  ...
  def self.print_detail(cs, i, sio)
    ...
    if i.reads_reg?('sp') || i.reads_reg?(12) || i.reads_reg?(REG_SP)
      raise 'Error in reg read decomposition' unless i.reads_reg?('sp') && i.reads_reg?(12) && i.reads_reg?(REG_SP)
    end
    if i.writes_reg?('lr') || i.writes_reg?(10) || i.writes_reg?(REG_LR)
      raise 'Error in reg write decomposition' unless i.writes_reg?('lr') && i.writes_reg?(10) && i.writes_reg?(REG_LR)
    end
    ...
    if i.op_count > 0
      ...
      i.operands.each.with_index do |op, idx|
        ...
        when OP_FP
          sio.puts "\t\toperands[#{idx}].type: FP = 0x#{uint32(op[:value][:fp])}"
        when OP_CIMM
          sio.puts "\t\toperands[#{idx}].type: C-IMM = #{uint32(op[:value][:imm])}"
        when OP_PIMM
          sio.puts "\t\toperands[#{idx}].type: P-IMM = #{uint32(op[:value][:imm])}"
        when OP_SETEND
        ...
        when OP_MEM
          ...
          if op[:value][:mem][:scale] != 1
            sio.puts format("\t\t\toperands[#{idx}].mem.scale = %u", op[:value][:mem][:scale])
          end
          if op[:value][:mem][:disp] != 0
            sio.puts "\t\t\toperands[#{idx}].mem.disp: 0x#{uint32(op.value[:disp]).to_s(16)}"
          end
        else
        ...
        sio.puts "\t\toperands[#{idx}].vector_index = #{op[:vector_index]}" if op[:vector_index] != -1

        sio.puts "\t\tSubtracted: True\n" if op[:subtracted]
      end
    end

    sio.puts "\tCode condition: #{i.cc}" unless [CC_AL, CC_INVALID].include? i.cc
